...
class Optimizer:
  ...
  def __init__(self, name, lr=0.001, gama=0.9, beta_m=0.9, beta_v=0.999, epsilon=1e-8):
...
class MultiClassLogisticRegression:
  ...
  def fit(self, X, y,
          batch_size=64, lr=0.001, gama=0.9, beta_m=0.9, beta_v=0.999, epsilon=1e-8,
          rand_seed=4, verbose=False, optimizer='sgd'):
    ...
    opt = Optimizer(optimizer, lr=lr, gama=gama, beta_m=beta_m, beta_v=beta_v, epsilon=epsilon)
    
    i, update = 0, 0
    while (i < self.n_iter):
      self.loss.append(self.cross_entropy(self.y_one_hot_encoded, self.predict_with_X_aug_(X)))
      "*** YOUR CODE STARTS HERE ***"
      ...
      if i % 1000 == 0 and verbose: 
          print(' Training Accuray at {} iterations is {}'.format(i, self.evaluate_(X, self.y_one_hot_encoded)))
      i +=1
    return self
